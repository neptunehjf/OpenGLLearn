...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// view行列（視点変換行列）
		mat4 view;
		view = translate(view, vec3(0.0f, 0.0f, -3.0f)); // 整个场景向前(-z)移动，相当于摄像机向后(+z)移动了
														 // ワールド空間を-Z方向に移動 = カメラを+Z方向に移動
		myShader.SetMat4("uni_view", view);
		// 投影矩阵
		// projection行列（射影変換行列）
		mat4 projection;
		projection = perspective(radians(45.0f), (float)(WINDOW_WIDTH / WINDOW_HEIGHT), 0.1f, 100.0f);
