...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// バッファ書き込み後、書き込み禁止にして本番のステンシルテスト開始
		//glClear(GL_DEPTH_BUFFER_BIT); // 清空深度缓存，这样边框可以有透视效果
										// 深度バッファをクリア（輪郭に透視効果を付与）
		glStencilMask(0x00);//关闭写操作
							// 書き込み操作を無効化
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF); // 只有Stencil buffer不等于1的时候可以绘制
											 // Stencil bufferの値が1でない場合のみ描画許可
