...
#define WIDTH 800
#define HEIGHT 600
#define DEFAULT_MIX_WEIGHT 0.5

// 混合权重
// ブレンドウェイト
float mixValue = DEFAULT_MIX_WEIGHT;
...
int main()
{
	...
	// bugfix 初始状态混合不正确，因为在检测到键盘输入之前没有把初始值赋给shader
	// bugfix 初期ブレンド状態の不正。キーボード入力検知前にシェーダーへ初期値設定が欠落していた事象に対処
	myShader.SetFloat("uni_mixValue", mixValue);
...
void processInput(GLFWwindow* window, const Shader &shader)
{
	...
	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
	{
		...
		if (mixValue >= 1.0)
			mixValue = 1.0;
		shader.SetFloat("uni_mixValue", mixValue);
	}
