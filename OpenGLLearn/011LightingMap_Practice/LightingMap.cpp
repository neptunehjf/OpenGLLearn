...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		myShader.SetInt("material.glow", 2);
		...
		//片段着色器会根据GL_TEXTURE2读取texture_specular的贴图数据
		//フラグメントシェーダーがGL_TEXTURE2からtexture_specularをサンプリング  
	...
	glDeleteTextures(1, &texture_glow);
	myShader.Remove();
	lampShader.Remove();
