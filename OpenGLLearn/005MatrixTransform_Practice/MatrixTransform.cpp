...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 矩阵变换 
		glm::mat4 transMatrix1; // 注意这个变量必须在每次Draw前重新初始化，否则值会积累起来
		transMatrix1 = glm::translate(transMatrix1, glm::vec3(-0.5, 0.5, 0));
		float value = sin(glfwGetTime());
		transMatrix1 = glm::scale(transMatrix1, glm::vec3(value, value, 0.0));
		glUniformMatrix4fv(glGetUniformLocation(myShader.ID, "uni_transMatrix"), 1, GL_FALSE, glm::value_ptr(transMatrix1));

		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

		// 矩阵变换 
		glm::mat4 transMatrix2; // 注意这个变量必须在每次Draw前重新初始化，否则值会积累起来
		transMatrix2 = glm::translate(transMatrix2, glm::vec3(0.5, -0.5, 0));
		transMatrix2 = glm::rotate(transMatrix2, (float)glfwGetTime(), glm::vec3(0.0, 0.0, 1.0));
		glUniformMatrix4fv(glGetUniformLocation(myShader.ID, "uni_transMatrix"), 1, GL_FALSE, glm::value_ptr(transMatrix2));
