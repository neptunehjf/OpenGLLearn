...
public:
	...
	Shader irradianceShader;	// サンプリング値からの拡散放射輝度算出用
	Shader prefilterShader;  	// 鏡面反射用プリフィルタ画像生成用
	Shader BRDFShader;			// BRDF積分マップ事前計算用  
...
void Scene::CreateShader()
{
	...
	irradianceShader = Shader("GetEquireColor.vs", "PBR_Irradiance.fs");
	prefilterShader = Shader("GetEquireColor.vs", "PBR_Prefilter.fs");
	BRDFShader = Shader("PBR_BRDF.vs", "PBR_BRDF.fs");
}
...
bool Scene::LoadTexture(const string&& filePath, GLuint& texture, const GLint param_s, const GLint param_t)
{
	...
	else if (channel == 3)
	{
		...
		informat = GL_RGB;  // bugfix 法線マップ既に線形空間であるため、GL_RGBにする。
