...
// Gバッファ SSAO入力（Gバッファの出力、SSAO処理の入力）
GLuint fbo_SSAO = 0;                 
GLuint tbo_SSAO_posdepth = 0;   //  位置+深度情報
GLuint tbo_SSAO_normal = 0;      // 法線情報
GLuint tbo_SSAO_albedo = 0;      // アルベド情報
GLuint rbo_SSAO = 0;       

// SSAOバッファ SSAO出力（SSAO処理の出力）
GLuint fbo_SSAO_out = 0;    
GLuint tbo_SSAO_out = 0;        // オクルージョン情報

// SSAOブラーバッファ（ノイズ平滑化用）
GLuint fbo_SSAO_blur = 0;      
GLuint tbo_SSAO_blur = 0;       //平滑化済みオクルージョン情報


// PBR IBL 環境Cubemap
GLuint fbo_EnvCubemap = 0;      
GLuint tbo_EnvCubemap = 0;           

// PBR IBL 放射Cubemap (IBL diffuse部分)
GLuint fbo_irdCubemap = 0;         
GLuint tbo_irdCubemap = 0;        

// PBR IBL pre filter cubemap　(IBL specular部分1)
GLuint fbo_pfCubemap = 0;
GLuint tbo_pfCubemap = 0; 

// PBR IBL BRDF (IBL specular部分2)
GLuint fbo_BRDF = 0;
GLuint tbo_BRDF = 0;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//if (bSSAO)
		//{
			//　参照Referrence/SSAO pass.png
			//　 原シーンを入力　g-infoをSSAO g-bufferに出力
		//	glBindFramebuffer(GL_FRAMEBUFFER, fbo_SSAO);
		//	scene.DrawScene_SSAOTest();
			//　 SSAO g-bufferを入力　occlusionをfbo_SSAO_outに出力
		//	glBindFramebuffer(GL_FRAMEBUFFER, fbo_SSAO_out);
		//	DrawSceneSSAO();
			//　 SSAO g-bufferを入力　occlusion_blurを fbo_SSAO_blurに出力
...
void CreateFrameBuffer_G()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	 ...
	 // 用GL_CLAMP_TO_EDGE防止采样到屏幕空间之外的深度值
	 // GL_CLAMP_TO_EDGEを使用してスクリーン空間外の深度値サンプリングを防止
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glBindTexture(GL_TEXTURE_2D, 0);
...
void DrawSceneDeffered()
{
	...
	// 原シーンをtbo_G_position tbo_G_normal tbo_G_abdspec　tbo_SSAO_blurとして出力
...
void CreateFrameBuffer_EnvCubemap()
{
	...
	// color附件等到渲染的时候再关联
	// color attachmentはレンダリング時にアタッチ

	// depth attachment　不要

	// 因为要等到渲染的时候关联纹理附件，所以此处不判断完整性了
	// レンダリング時にcolor attachmentを関連付けるため、ここでは完全性チェックをスキップ
...
void DrawEnvCubemap(Mesh &cube)
{
	...
	for (uint i = 0; i < 6; i++)
	{
		...
		// 
		// 立方体マップ面ごとにcolor attachmentをバインド
		// ジオメトリシェーダーで6面を一括描画も可能だが、パフォーマンス低下のリスクあり
	...
	// 因为这里想要自动生成的mipmap, 因此 这里在绘制环境cubemap之后再调用glGenerateMipmap，而不是在分配缓冲的时候调用
	//
	// 注意: glGenerateMipmap呼び出しは対応するミップマップメモリを生成しようと試みますが、
	// ベースイメージ未描画の状態で呼び出すと黒色のみ出力されます
	// 本処理では自動生成ミップマップを有効活用するため、
	// 環境キューブマップ描画完了後にglGenerateMipmapを実行（バッファ割当時ではなく）
	glBindTexture(GL_TEXTURE_CUBE_MAP, tbo_EnvCubemap);
...
void SkyBoxTest(Mesh& skybox)
{
	...
	// GL_BLEND有効時、アルファチャンネルがないため物体が表示されない可能性あり、要無効化
	glDisable(GL_BLEND);
...
// PBR IBL pre filter cubemap　(IBL specular部分1)　の framebufferを作成
void CreateFrameBuffer_PrefilterCubemap()
{
	...
	//
	// プリフィルタ環境マップでは粗さとMIPレベル対応関係を制御する必要があるため
	// ベース画像描画後glGenerateMipmapで自動生成すると要件を満たせません
	// ここで(ベース画像描画前に)glGenerateMipmapを実行しMIPMAPメモリを初期化(黒状態)
	// 後続処理で各MIPレベルに適切なデータを手動生成します
	glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
...
// PBR IBL pre filter cubemap　(IBL specular部分1)　を描画
void DrawPrefilterCubemap()
{
	// projection
	float aspect = 1.0f;
...
// PBR IBL BRDF (IBL specular部分2)の framebufferを作成
void CreateFrameBuffer_BRDF()
{
	glGenFramebuffers(1, &fbo_BRDF);
	glBindFramebuffer(GL_FRAMEBUFFER, fbo_BRDF);

	/**********************color buffer，BRDF LUTを格納*********************/

	glGenTextures(1, &tbo_BRDF);

	glBindTexture(GL_TEXTURE_2D, tbo_BRDF);
	// BRDF用双通道float即可
	// BRDF用2チャンネルfloatテクスチャ（16bit浮動小数点で十分）
...
// PBR IBL BRDF (IBL specular部分2)を描画
void DrawBRDF()
{
	glBindFramebuffer(GL_FRAMEBUFFER, fbo_BRDF);
